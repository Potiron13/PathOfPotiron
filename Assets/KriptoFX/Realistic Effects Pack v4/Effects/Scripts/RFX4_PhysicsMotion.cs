...
public class RFX4_PhysicsMotion : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.collider.tag == transform.parent.tag) return;
        if (collision.collider.GetComponent<PlayerData>() == null) return;
        Collider[] colliders = Physics.OverlapSphere(transform.position, 1f);
        foreach (Collider collider in colliders)
        {
            PlayerData playerData = collider.GetComponent<PlayerData>();
            if (playerData != null && collider.tag != transform.parent.tag)
            {
                collider.GetComponent<PlayerData>().Attack(attacker.character, ability);
            }
        }        
